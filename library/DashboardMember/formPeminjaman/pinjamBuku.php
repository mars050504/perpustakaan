...
// Periksa apakah pengguna sudah masuk. Jika tidak, alihkan ke halaman sign in
...
$nisnSiswa = isset($_SESSION["member"]["nisn"]) ? $_SESSION["member"]["nisn"] : null;
$dataSiswa = queryReadData("SELECT * FROM member WHERE nisn = '$nisnSiswa'");

// Ambil data admin
$admin = queryReadData("SELECT * FROM admin");

// Tangani aksi peminjaman buku saat form disubmit
if(isset($_POST["pinjam"])) {
  $idBuku = $_POST["id_buku"];
  $nisn = $_POST["nisn"];
  $namaAdmin = $_POST["nama_admin"]; // Menggunakan $_POST["nama_admin"] untuk mendapatkan nama admin
  $tglPinjam = $_POST["tgl_peminjaman"];
  $tglKembali = $_POST["tgl_pengembalian"];

  // Dapatkan ID admin berdasarkan nama admin yang dipilih
  $queryGetAdminId = "SELECT id FROM admin WHERE nama_admin = '$namaAdmin'";
  $resultAdminId = mysqli_query($connection, $queryGetAdminId);
  
  if ($resultAdminId) {
      $adminData = mysqli_fetch_assoc($resultAdminId);
      $idAdmin = $adminData["id"];
  } else {
      // Handle error jika tidak ditemukan nama admin
      echo "Error: " . mysqli_error($connection);
  }

  // Selanjutnya, menggunakan nilai $idAdmin yang diperoleh untuk menyimpan data peminjaman buku ke database:
...
mysqli_query($connection, $queryPinjam);
  ...
  // Panggil fungsi untuk melakukan peminjaman buku
  $result = pinjamBuku($_POST);

  // Periksa apakah peminjaman berhasil
if($result) {
  echo "<script>
          alert('Buku berhasil dipinjam');
        </script>";

  // Alihkan pengguna kembali ke daftarBuku.php setelah pesan berhasil ditampilkan
...
<html lang="en">
  ...
  <div class="p-4">
    ...
    <!--Ambil data id admin-->
    <select name="nama_admin" class="form-select" aria-label="Default select example">
      <option selected>Pilih nama admin</option>
        <?php foreach ($admin as $item) : ?>
      <option><?= $item["nama_admin"]; ?></option>
        <?php endforeach; ?>
    </select>
